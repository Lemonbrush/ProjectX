...
export (bool) var is_skippable_part_finished = false
...
var tempPlayer
var is_able_to_skip_cutscene = false

func _ready():
	is_able_to_skip_cutscene = SettingsManager.settings.is_intro_skip_option_available
	var _connection = portal_interaction_object.connect("interacted_with_arg", self, "player_interacted_with_portal")

func _unhandled_input(event: InputEvent):
	if event.is_action_pressed("Interaction") && !is_skippable_part_finished && is_able_to_skip_cutscene:
...
func show_skip_popup_if_needed():
	if is_able_to_skip_cutscene:
		skip_hint_popup.show()

func hide_skip_intro_popup():
	skip_hint_popup.hide()
